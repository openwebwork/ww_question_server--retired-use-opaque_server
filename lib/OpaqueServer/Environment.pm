package OpaqueServer::Environment;
...
sub new {
    ...
    # Compile the "include" function with all opcodes available.
		...
		my ($file) = @_;
		my $fullPath = "].$OpaqueServer::RootDir.q[/$file";
    ...
    #die $preps;
    #$safe->reval($preps);
    my $globalEnvironmentFile = $OpaqueServer::RootDir . "/conf/global.conf";
    my $globalFileContents = readFile($globalEnvironmentFile);

    my $settings = "\$OpaqueServer::RootDir = '" . $OpaqueServer::RootDir . "';\n";
    $settings .= "\$OpaqueServer::RootPGDir = '" . $OpaqueServer::RootPGDir . "';\n";
    $settings .= "\$OpaqueServer::Host = '" . $OpaqueServer::Host . "';\n";
    $settings .= "\$OpaqueServer::FilesURL = '" . $OpaqueServer::FilesURL . "';\n";
    $safe->reval($settings);
